...
const BOOL CSQLite3::ExecuteSQL(IN const CString& strSQL, IN const BOOL& bIgnorePrimaryKey)
{
	...
	try
	{
		...
		if (0 == strSQL.Left(6).CompareNoCase(_T("SELECT")))
		{
			...
			{
				iDBStatus = sqlite3_finalize(m_pSQLite3STMT);
				m_pSQLite3STMT = NULL;
			}
			//尝试多次执行指定SQL
			while (bContinueExecuteSQL)
			{
				iDBStatus = sqlite3_prepare_v2(m_pSQLite3DB, strRealSQL.GetBuffer(0), strRealSQL.GetLength(), &m_pSQLite3STMT, NULL);
				switch (iDBStatus)
				{
				case SQLITE_BUSY:
					::Sleep(50);
		...
		else
		{
			...
			while (bContinueExecuteSQL)
			{
				iDBStatus = sqlite3_exec(m_pSQLite3DB, strRealSQL.GetBuffer(0), NULL, NULL, NULL);
				switch (iDBStatus)
				{
				case SQLITE_BUSY:
					::Sleep(50);
	...
	catch (int& iErrorCode)
	{
		...
		m_bConfirmHasRecordSet = FALSE;
		//获得数据库反馈的错误与代码
		m_dwLastErrorCode = sqlite3_errcode(m_pSQLite3DB);
		strcpy_s(szThrowDBError, _countof(szThrowDBError), sqlite3_errmsg(m_pSQLite3DB));
		switch (iErrorCode)
		{
		case -1:
...
const BOOL CSQLite3::OpenConnection(IN const BOOL& bAutoShrinkageDatabase)
{
	...
	try
	{
		...
		if (NULL == pszString)
			throw - 3;
		//执行创建或打开SQLite3数据库指令
		iDBStatus = sqlite3_open_v2(pszString, &m_pSQLite3DB, m_dwOpenFlag, NULL);
		if (SQLITE_OK != iDBStatus)
			throw - 4;
...
const BOOL CSQLite3::CloseConnection(void)
{
	...
	try
	{
		...
		{
			iDBStatus = sqlite3_close_v2(m_pSQLite3DB);
			if (SQLITE_OK != iDBStatus)
				throw - 1;
			m_pSQLite3DB = NULL;
		}
...
const int CSQLite3::EffectRows(IN const CString& strExecuteSQL)
{
	...
	if (IsOpenConnection())
	{
		...
		{
			iEffectRows = sqlite3_changes(m_pSQLite3DB);
		}
		else
		{
			...
			if (SQLITE_OK == (iDBStatus = sqlite3_prepare_v2(m_pSQLite3DB, strSQL.GetBuffer(0), strSQL.GetLength(), &pSQLite3STMT, NULL)))
			{
				if (SQLITE_ROW == (iDBStatus = sqlite3_step(pSQLite3STMT)))
				{
					int iRecordCount = -1;
					//获取指定表的字段数目
					int iFieldCount = sqlite3_column_count(pSQLite3STMT);
					//字段数目大于0,表示数据存在,开始解析读取
					for (int i = 0; i < iFieldCount; ++i)
					{
						char szFieldName[128] = {0};
						strcpy_s(szFieldName, _countof(szFieldName), sqlite3_column_name(pSQLite3STMT, i));
						if (0 == strColumnName.CompareNoCase(szFieldName))
						{
							int iDataType = sqlite3_column_type(pSQLite3STMT, i);
							if (SQLITE_INTEGER == iDataType)
							{
								iRecordCount = sqlite3_column_int(pSQLite3STMT, i);
			...
			if (NULL != pSQLite3STMT)
				iDBStatus = sqlite3_finalize(pSQLite3STMT);
			pSQLite3STMT = NULL;
		}
	}
	return iEffectRows;
...
{
	int iDBStatus = -1;
	if (SQLITE_ROW == (iDBStatus = sqlite3_step(m_pSQLite3STMT)))
	{
...
const BOOL CSQLite3::GetColumnValue(IN const CString& strColumnName, IN OUT CString& strValue)
{
	...
	try
	{
		if (NULL == m_pSQLite3STMT)
			throw - 1;
		//获取指定表的字段数目
		int iFieldCount = sqlite3_column_count(m_pSQLite3STMT);
		//字段数目大于0,表示数据存在,开始解析读取
		if (0 == iFieldCount)
			throw - 2;
		for (int i = 0; i < iFieldCount; ++i)
		{
			char szFieldName[128] = {0};
			strcpy_s(szFieldName, _countof(szFieldName), sqlite3_column_name(m_pSQLite3STMT, i));
			if (0 == strColumnName.CompareNoCase(szFieldName))
			{
				int iDataType = sqlite3_column_type(m_pSQLite3STMT, i);
				if (SQLITE_TEXT == iDataType)
				{
					bResult = TRUE;
					strValue.Format(_T("%s"), sqlite3_column_text(m_pSQLite3STMT, i));
				}
				break;
			}
		}
	...
	try
	{
		...
		for (int i = 0; i < iFieldCount; ++i)
		{
			...
			if (0 == strColumnName.CompareNoCase(szFieldName))
			{
				...
				if (SQLITE_INTEGER == iDataType)
				{
					iValue = sqlite3_column_int(m_pSQLite3STMT, i);
	...
	try
	{
		...
		for (int i = 0; i < iFieldCount; ++i)
		{
			...
			if (0 == strColumnName.CompareNoCase(szFieldName))
			{
				...
				if (SQLITE_INTEGER == iDataType)
				{
					nValue = (UINT)sqlite3_column_int(m_pSQLite3STMT, i);
	...
	try
	{
		...
		for (int i = 0; i < iFieldCount; ++i)
		{
			...
			if (0 == strColumnName.CompareNoCase(szFieldName))
			{
				...
				if (SQLITE_INTEGER == iDataType)
				{
					llValue = sqlite3_column_int64(m_pSQLite3STMT, i);
	...
	try
	{
		...
		for (int i = 0; i < iFieldCount; ++i)
		{
			...
			if (0 == strColumnName.CompareNoCase(szFieldName))
			{
				...
				{
					dValue = sqlite3_column_double(m_pSQLite3STMT, i);
