	...
	else//版本非0，扣费处理为首次扣费，后续循环查询
	{
		...
		for(int i=0; i<iLoop; ++i)
		{
			...
			try
			{
				...
				//根据发送过来的数据类型，进行数据格式和转化，并获得服务端链接信息
				if(!DataConvertFromJson2String(wSendDataType,pDVJsonReceive,strURL,wPort,strSendData,hWnd,iCallMode,&pJsonIF, iDealSN))
				{
					bLoopBreak = TRUE;
					throw -2;
				}
...
//     
////////////////////////////////////////////////////////////////////////////////////////////////
const BOOL CThreadHttp::DataConvertFromJson2String(IN const WORD& wSendDataType,IN OUT CDVJsonBase* pDVJsonReceive,IN OUT CString& strURL,IN OUT INTERNET_PORT& wPort,IN OUT CString& strSendData,IN OUT HWND& hWnd,IN OUT int& iCallMode,IN OUT CJsonOperationIF** pJsonIF, int nSN )
{
	BOOL bReturn = TRUE;
