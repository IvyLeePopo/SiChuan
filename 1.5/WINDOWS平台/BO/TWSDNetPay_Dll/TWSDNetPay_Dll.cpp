...
int				CTWSDNetPay_DllApp::m_iEnableSystemSwitch						= 0;
...
void CTWSDNetPay_DllApp::ReadConfigInfo(void)
{
	...
	//是否使能系统切换（内外网模式切换）
	m_iEnableSystemSwitch = ::GetPrivateProfileInt(_T("SetUp"), _T("EnableSystemSwitch"), 0, strConfigFilePath);
}
...
int CTWSDNetPay_DllApp::GetAgencyResult(CString strXML, DWORD dwConsumeTime)
{
    ...
    try
    {
        if ( xml.SetDoc( strXML ) )
        {
            ...
            {
            case 0x07:
                m_sCurrentDebitResult.PayPlatformType = PCTC_WEIXIN_AGENCY;//PCTC_ALIIPAY_AGENCY;
				if(41==m_sTranstParam.ProvinceID)
				{//add 2018-03 河南代扣返回码特殊处理
