...
// 	return true;
	...
	try
	{
		...
		//启动版本管理线程
		CString strLog;			
		if(GetGlobalApp()->m_iEnableSystemSwitch != 0)
		{
			strLog.Format(_T("[业务管理线程]: >>>>>>使用系统切换（内外网模式切换）功能"));
		...
		{
			strLog.Format(_T("[业务管理线程]: >>>>>>不使用系统切换（内外网模式切换）功能"));
		}
		GetGlobalApp()->RecordLog(strLog);
	}
	catch (int& iErrorCode)
