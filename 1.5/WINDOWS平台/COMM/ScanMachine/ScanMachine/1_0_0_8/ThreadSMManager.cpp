...
void CThreadSMManager::fnReadSpecailConfig(void)
{
	...
	//获取串口异常后需要复位的间隔时间
	m_iResetCOMErrorSpanTime = ::GetPrivateProfileInt(_T("SMControl"),_T("ResetCOMErrorSpanTime"),30,m_strPathLocalConfigFile);
...
DWORD CThreadSMManager::WriteDataToComm(IN const LPBYTE byData,IN const DWORD& dwCommandDataSize)
{
	...
	if( INVALID_HANDLE_VALUE != m_hCom )
	{
		if(!(::WriteFile(m_hCom,byData,dwCommandDataSize,&dwWriteDataLen,NULL)))
		{
			...
			{
				theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,检测串口下发数据状态[故障]")));
			}
			else
			{
				theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,发送数据到设备失败，错误代码:%d"),GetLastError()));
			}
		}
		else
		{
			...
			if( (1 == dwCommandDataSize) && (strCmd == _T("FF")) )
			{
				theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,检测串口下发数据状态[正常]")));
			}
			else
			{
				theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,发送数据到设备:%d -- %s"),dwCommandDataSize,strCmd));
			}
		}
	}
	else 
	{
		//串口句柄异常
		theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,接收到发送数据的请求，内部错误:串口句柄异常(%d -- %s)"),dwCommandDataSize,strCmd));
	}
	//返回实际发出去的字节数
...
BOOL CThreadSMManager::WriteDataToComm(IN const CString& strWriteData)
{
	...
	{
		//转译成字节流
		Hex2Bin(strRealData.GetBuffer(0),byData,strRealData.GetLength());
...
BOOL CThreadSMManager::OnIdle(LONG lCount)
{
		...
		)
	{
		...
		if( (time(NULL) - tLastSendTestCharTime) > 10)
		{
			...
			if(!WriteDataToComm(_T("FF")))
			{
				...
				{
					//记录日志
					theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,串口[端口:%d]状态出现异常（限定时间内未成功[收到/发送]任何数据），需要进行重置"),m_sLocalCOM.Port));
					//关闭串口连接
					CloseCom();
					//标记上次状态
					m_bCOMLastOpenState = FALSE;
					...
					if(!m_bOpen)
					{
						theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,串口[端口:%d]打开失败"),m_sLocalCOM.Port));
					}
					//记录操作时间
	...
	if(m_bCOMLastOpenState != m_bOpen)
	{
		...
		//记录日志
		theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,打开本地串口[端口:%d,波特率:%d,数据位:%d,停止位:%d,校验位:%d] %s"),m_sLocalCOM.Port,m_sLocalCOM.BaudRate,m_sLocalCOM.DataBit,m_sLocalCOM.StopBit,m_sLocalCOM.ParityBit,m_bOpen?_T("成功"):_T("失败")));
	}
	...
	if(NULL != m_hCom)
	{
		...
		if(comStat.cbInQue>0)//接收缓冲区中有数据
		{
			...
			BYTE	byRBuff[512] = {0};					//接收数据缓冲区(定义512字节)
			BOOL	bHaveFram = FALSE;

			CString strReceiveData;
			while(!bHaveFram)
			{
				//读取所能允许的最大字节数(每次读取最大数目为动态上限)
				ReadFile(m_hCom,&byRBuff[dwEnd],512 - dwEnd,&dwReadNum,NULL); 
				if(dwReadNum>0)
				{
					//更新当前读取数据时间
				...
				{ 
					if((::timeGetTime() - dwNewTime) > m_iNoNewDataTime)//连续20毫秒(默认)没有读到数据，函数返回
					{
						//保存已收到的数据包
						if(dwEnd>=1)
						{
							...
							//if(   (NULL != m_pSMProtocol)
							//	&&(NULL != m_pSMProtocol->GetInnerDeviceProtocolClass())
							//	&&((SM_CMD_START == m_pSMProtocol->GetInnerDeviceProtocolClass()->GetLastCommandType()) || (0x57 == m_pSMProtocol->GetInnerDeviceProtocolClass()->GetLastCommandType()))
							//	)
							//{
							//	//收到大于3个字节的无效数据，判定扫码失败，通知调用者
			...
			if(m_pSMProtocol->ProcessReceiveFrameData(&byRBuff[dwStart],dwOrgDataLen))
			{
...
void CThreadSMManager::OnMsgStartScan(WPARAM wParam,LPARAM lParam)
{
	...
	//记录日志
	theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,接收到命令,开始进行扫码预处理操作")));
	//控制灯光
...
{
	//记录日志
	theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,接收到命令,通知工作线程停止扫码")));
	//控制灯光
	fnSMControlLight(CHTSMDeviceControl::SM_PRPTOCOL_LIGHT_ALL_OFF);
	//延时控制
	Sleep(m_dwSpanDelayTimeForControlLight);
...
void CThreadSMManager::OnMsgDealResponseForProtocolProcessResult(WPARAM wParam,LPARAM lParam)
{
	...
	{	
		...
		case SM_CMD_RESET_PERSION:
			{
				strLog = FmtStr(_T("扫码管理线程,%s"),pSMRemoteState->StateDesc);
			}
			break;
		case SM_CMD_START:
			{
...
void CThreadSMManager::fnSMScanCode(void)
{
	...
	try
	{
		...
		//获取内挂设备的扫码命令内容
		strInnerDeviceCommand = FmtStr(_T("%02X%s"),SM_CMD_START,m_pSMProtocol->GetInnerDeviceProtocolClass()->Start());
		//获取格式化发送数据
		...
		theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,%s"),strError));	
...
void CThreadSMManager::fnSMStopScan(void)
{
	...
	try
	{
		...
		if(NULL == pSMIF)
			throw -2;
		//获取停止扫描命令
		strInnerDeviceCommand = FmtStr(_T("%02X%s"),SM_CMD_STOP,pSMIF->Stop());
		...
		if(!WriteDataToComm(strSendData))
			throw -4;
		//写日志
		theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,发送强制停止扫码指令[成功]")));
	}
	catch(int& iErrorCode)
	{
		switch(iErrorCode)
		...
		//写日志
		theApp.RecordMsgTemp(FmtStr(_T("扫码管理线程,发送强制停止扫码指令[失败]")));
	}
}
