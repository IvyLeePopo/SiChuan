...
BOOL CThreadSMManager2::InitInstance()
{
	...
	m_iResetCOMErrorSpanTime = time(NULL);
...
{	
	if(NULL != m_pCOM)
	{
		...
		CString strDesc = FmtStr(_T("本地串口[端口号:%d,波特率:%d,停止位:%d,校验位:%d,数据位:%d]打开%s"),m_sLocalCOM.Port,m_sLocalCOM.BaudRate,m_sLocalCOM.StopBit,m_sLocalCOM.ParityBit,m_sLocalCOM.DataBit,m_bOpen?_T("成功"):_T("失败"));
...
DWORD CThreadSMManager2::WriteDataToComm(IN const LPBYTE byData,IN const DWORD& dwCommandDataSize)
{
	...
	if(NULL != m_pCOM)
	{
		...
		if(m_bPrintCommandDataInLog)
		{
			...
			theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,发送数据到设备:%d -- %s"),dwCommandDataSize,strCmd));
		}
	}
...
BOOL CThreadSMManager2::OnIdle(LONG lCount)
{
		...
		)
	{
		...
		if( (time(NULL) - tLastOpenCOMTime) > 30)
		{
			//记录日志
			theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,串口[端口:%d]状态出现异常（限定时间内未成功[收到]任何数据），需要进行重置"),m_sLocalCOM.Port));
			//关闭串口连接
			CloseCom();
			//标记上次状态
			m_bCOMLastOpenState = FALSE;
			...
			if(!m_bOpen)
			{
				theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,串口[端口:%d]打开失败"),m_sLocalCOM.Port));
			}
			//记录操作时间
	...
	if(m_bCOMLastOpenState != m_bOpen)
	{
		...
		//记录日志
		theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,打开本地串口[端口:%d,波特率:%d,数据位:%d,停止位:%d,校验位:%d] %s"),m_sLocalCOM.Port,m_sLocalCOM.BaudRate,m_sLocalCOM.DataBit,m_sLocalCOM.StopBit,m_sLocalCOM.ParityBit,m_bOpen?_T("成功"):_T("失败")));
	}
	...
	{
	//	theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,线程存活检测日志记录，当前任务编码 %d"),::GetGlobalApp()->m_nTask));
...
void CThreadSMManager2::OnMsgStartScan(WPARAM wParam,LPARAM lParam)
{
	...
	//记录日志
	theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,接收到命令,开始进行扫码预处理操作")));
	//控制灯光
...
{
	//记录日志
	theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,接收到命令,停止扫码")));
	//控制灯光
	fnSMControlLight(CHTSMDeviceControl::SM_PRPTOCOL_LIGHT_ALL_OFF);
	//延时控制
	Sleep(m_dwSpanDelayTimeForControlLight);
...
void CThreadSMManager2::OnMsgDataSendEnd(WPARAM wParam,LPARAM lParam)
{
	...
	if(NULL != m_pSMProtocol)
	{
		if(NULL != m_pSMProtocol->GetInnerDeviceProtocolClass())
		{
			int iCmdType = m_pSMProtocol->GetInnerDeviceProtocolClass()->GetLastCommandType();
			switch(iCmdType)
			{
			case SM_CMD_START:
				//扫码命令发送完毕
...
void CThreadSMManager2::OnMsgReceiveDataFromCOM(WPARAM wParam,LPARAM lParam)
{
	...
	if(NULL != m_pCOM)
	{
		...
		if((dwReadBytes = m_pCOM->Read(m_lpReceiveDataBuff + m_dwReceiveDataIndex,dwSize))>0)
		{
			...
			//进制转换处理
			CString strTemp(Bin2Hex(m_lpReceiveDataBuff + m_dwReceiveDataIndex,dwReadBytes));
			...
			{
				strReceiveData += FmtStr(_T("%02X "),*pBegin++);
			}
			pBegin = byTemp;
...
void CThreadSMManager2::OnMsgDealResponseForProtocolProcessResult(WPARAM wParam,LPARAM lParam)
{
	...
	{	
		...
		case SM_CMD_RESET_PERSION:
			{
				strLog = FmtStr(_T("扫码工作线程,%s"),pSMRemoteState->StateDesc);
			}
			break;
		case SM_CMD_START:
			{
				...
				{
					strLog = FmtStr(_T("扫码工作线程,操作步骤3:该次扫码完毕,扫码器[%d]获取数据:%s"),pSMRemoteState->DeviceID,pSMRemoteState->Code);
				}
				else
				{
					strLog = FmtStr(_T("扫码工作线程,操作步骤3:该次扫码完毕,未获取任何有效数据"));
				}
...
void CThreadSMManager2::fnSMStopScan(void)
{
	...
	try
	{
		...
		if(!WriteDataToComm(strSendData))
			throw -4;
		//写日志
		theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,发送强制停止扫码指令[成功]")));
	}
	catch(int& iErrorCode)
	{
		switch(iErrorCode)
		...
		//写日志
		theApp.RecordMsgTemp(FmtStr(_T("扫码工作线程,发送强制停止扫码指令[失败]")));
	}
}
