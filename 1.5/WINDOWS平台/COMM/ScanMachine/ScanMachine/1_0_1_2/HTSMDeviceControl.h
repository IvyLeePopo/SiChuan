...
public:
	...
	//处理接收数据
	const BOOL ProcessReceiveFrameData(IN const LPBYTE lpData,IN const DWORD& dwSize);

	//读取串口板配置
	const CString ReadConfigInfoFromBoard(void);
	//更新串口板配置
...
private:
	...
	//数据转译
	void CovertData(IN OUT LPBYTE lpData,IN OUT DWORD& dwSize,IN const BOOL& bSendState);
	//获取校验码
