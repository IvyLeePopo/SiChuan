...
BOOL WINAPI IF_GetDeviceFirewareVer(IN OUT CString& strXML)
{
	...
	if( bInitDev )
	{
		csErrorMsg.Format( _T("IF_GetDeviceFirewareVer:	获取扫码器固件参数数据失败,暂不支持！") );
		return FALSE;

		if(NULL != theApp.m_pThreadSMManager)
		{
			csErrorMsg.Format( _T("IF_GetDeviceFirewareVer:	获取扫码器固件参数数据成功！") );
			return TRUE;
		}
		else
		{
			csErrorMsg.Format( _T("IF_GetDeviceFirewareVer:	扫码器控制线程无效！") );
			theApp.RecordMsgTemp( csErrorMsg );
			return FALSE;
		}
	}
...
BOOL WINAPI IF_IssuedData(IN const CString& strXML)
{
	...
	if( bInitDev )
	{
		...
		if(NULL != theApp.m_pThreadSMManager)
		{
			csErrorMsg.Format( _T("IF_IssuedData:	往智能硬件下发数据成功！") );
			return TRUE;
		}
		else
		{
