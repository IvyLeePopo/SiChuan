...
#define WM_THREAD_SM_TTS                 				WM_APP + MSG_START_ID_SM + 15	//TTS语音播报


#define WM_THREAD_REMOTE_UPGRADE_RESULT                 WM_APP  + 51					//升级结果，需要和业务动态库中的设置保持一致
...
{
	int		DealInfoUse;	//收费信息显示启用标记，非0有效
	...
	int		GGMsgUse;		//广告文字启用标记，非0有效
	TCHAR	GGMsg[128];		//广告文字信息
	int		GGVoiceUse;		//广告声音启用标记，非0有效
	TCHAR	GGVoice[128];	//广告声音信息
	int		GGWelSetUse;	//广告欢迎界面设置启用标记，非0有效
	TCHAR	GGWelSet[12];	//广告欢迎界面设置
};


struct tagTransmissionData
{
	int module;
	int iSerial;
	int type;
	...
	tagTransmissionData()
	{
		memset(this,0,sizeof(tagTransmissionData));
	}
};
struct tagTransmissionFeedback
{
	int DeviceID;
	TCHAR package[1000*2];
	tagTransmissionFeedback()
	{
		memset(this,0,sizeof(tagTransmissionFeedback));
	}
};
struct tagDeviceFeedback
{
	int feed_type;//1:QR code,2:NFC
	tagSMInnerDeviceState inner_dev;
	tagTransmissionFeedback trans_feedback;
	tagDeviceFeedback()
	{
		memset(this,0,sizeof(tagDeviceFeedback));
	}
};
