...
const BOOL CHTSDDeviceDll::Init(IN const CString& strDllFilePath)
{
	...
	if(NULL != (m_hModule = ::LoadLibrary(strDllFilePath)))
	{
		...
		fnIF_GetC0SuccFailCount		= (defIF_GetC0SuccFailCount)::GetProcAddress(m_hModule,"IF_GetC0SuccFailCount");
...
BOOL CHTSDDeviceDll::GetCSuccFailCount(OUT int& dwSucce, OUT int& dwFailed, OUT int& nReSendCount,OUT CString& strA2Frame, OUT CString& strC0Frame)
{
	if(m_bInitDll && m_bInitDev)
		return fnIF_GetC0SuccFailCount(dwSucce,dwFailed,nReSendCount,strA2Frame,strC0Frame);
	return FALSE;
}

BOOL CHTSDDeviceDll::DebitMoney(CString& csDebitInfo, int nType)
{
	if(m_bInitDll && m_bInitDev)
		return fnIF_DebitMoney(csDebitInfo,nType);
	return FALSE;
}

BOOL CHTSDDeviceDll::DebitCancel(CString& csCancelInfo, int nType)
{
	if(m_bInitDll && m_bInitDev)
		return fnIF_DebitCancel(csCancelInfo,nType);
	return FALSE;

}

BOOL CHTSDDeviceDll::InitConfigDevice(CString& csInitInfo, int nType)
{
	if(m_bInitDll && m_bInitDev)
		return fnIF_InitDeviecCMD(csInitInfo,nType);
	return FALSE;
}
