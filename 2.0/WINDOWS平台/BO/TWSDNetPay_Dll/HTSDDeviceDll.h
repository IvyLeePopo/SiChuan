...
public:
	...
	//test c0 frame
	typedef BOOL	(WINAPI  *defIF_GetC0SuccFailCount)(OUT int& dwSucce, OUT int& dwFailed, OUT int& nReSendCount, OUT CString& strA2Frame, OUT CString& strC0Frame);
	//执行扣款操作
	typedef BOOL    (WINAPI *defIF_DebitMoney)(IN const CString& csDebitInfo, IN const int nFormatType);
	//执行撤单操作
	typedef BOOL    (WINAPI *defIF_DebitCancel)(IN const CString& csDebitInfo, IN const int nFormatType);

	//执行设备初始配置操作
	typedef BOOL    (WINAPI *defIF_InitDeviecCMD)(IN const CString& csInitInfo, IN const int nFormatType);
...
public:
	...
	//test
	BOOL GetCSuccFailCount(OUT int& dwSucce, OUT int& dwFailed, OUT int& nReSendCount,OUT CString& strA2Frame, OUT CString& strC0Frame);

	//扣款操作
	BOOL DebitMoney(CString& csDebitInfo, int nType);
	//撤单操作
	BOOL DebitCancel(CString& csCancelInfo, int nType);
	//设备初始化配置操作
	BOOL InitConfigDevice(CString& csInitInfo, int nType);
...
private:
	...
	//Test
	defIF_GetC0SuccFailCount	fnIF_GetC0SuccFailCount;
