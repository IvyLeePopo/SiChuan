...
const BOOL CDVXMLDebitParamContext::Init(IN const CString& strXMLString)
{
	...
	try
	{
		...
		{
			AutoPlateColor = _ttoi(xml.GetChildData());
		}
		xml.OutOfElem();
		...
		if(!xml.FindChildElem(_T("StationID")))
			throw -22;
		EntryStationID = _ttoi(xml.GetChildData());
		str_EntryStationID = xml.GetChildData();
		...
		if(!xml.FindChildElem(_T("LaneID")))
			throw -25;
		EntryLane = _ttoi(xml.GetChildData());
		str_EntryLane = xml.GetChildData();
		...
		if(!xml.FindChildElem(_T("AreaID")))
			throw -34;
		EntryAreaID = _ttoi(xml.GetChildData());

		str_EntryAreaID = xml.GetChildData();
	
		xml.ResetChildPos();
		if(!xml.FindChildElem(_T("RoadID")))
			throw -35;
		EntryRoadID = _ttoi(xml.GetChildData());
		str_EntryRoadID = xml.GetChildData();
