...
void CSystemSwitch::SystemDetect()
{
	...
	{
		NotifyTerminalSwitch2V2_0();
		return ;
	}
	LogSome("Network is now available!");
	if(NotifyTerminalSwitch2V1_5())
	{
		LogSome("终端已切换至M1，动态库即将切换");
		ExcuteMiddlewareSwitch();
		m_InExchanging = true;
	}
}
bool CSystemSwitch::IsPCNetworkAvailable()
{
	...
	if(currTime-lastTestHeartBeatTime>5)
	{
		...
		std::string remoteIP = std::string((LPCSTR) GetGlobalApp()->m_sTranstParam.LaneConfigInfo.LocalPCWebServerIP);
