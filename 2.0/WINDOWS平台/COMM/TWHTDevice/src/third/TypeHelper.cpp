...
bool Hex2Bin(const char* hex_buf,std::list<unsigned char>&bin_buf,size_t hex_len)
{
    ...
    size_t tmpLen = hex_len/2;
    char* tmpChs = new char[tmpLen];
    if(tmpChs==NULL)
    {
        return false;
    }
    if(!Hex2Bin(hex_buf,tmpChs,hex_len))
    {
        delete[] tmpChs;
        return false;
    }
    std::copy(tmpChs,tmpChs+tmpLen,std::back_inserter(bin_buf));
    delete[]tmpChs;
    return true;
}


bool Hex2Bin(const char* hex_buf,std::vector<unsigned char>&bin_buf,size_t hex_len)
{
    if(hex_len>1024*100)
    {
        return false;
...
std::string Datetime2Str(time_t dt)
{
    ...
    char result[50]={0};
    tm1 = *localtime(&dt);
    sprintf(result,"%04d-%02d-%02d %02d:%02d:%02d",tm1.tm_year+1900,tm1.tm_mon+1,tm1.tm_mday,tm1.tm_hour,tm1.tm_min,tm1.tm_sec);
...
void RemoveWhiteSpaceInStr(std::string&s)
{
