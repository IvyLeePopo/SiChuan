...
CString TMP_MbcsToUtf8(const char *file)
{
    CString str;
    WCHAR   *pwchar=0;
    CHAR    *pchar=0;
    ...
    if(pwchar!=0)
    {
        ...
        if( len!=0 ) 
        {
            ...
            if(pchar!=0)
            {
                ...
                if(len!=0)                 
                {
                    str.Format(_T("%s"),pchar);;//str=pchar;                    
                }
                delete pchar;
            }
            delete pwchar; 
...
const BOOL CSQLite3::OpenConnection(IN const BOOL& bAutoShrinkageDatabase)
{
	...
	try
	{
		...
		//字符串转化
		//modify 
		CString tmpCst = TMP_MbcsToUtf8((LPSTR)(LPCTSTR)m_strDatabaseFilePath);
		char* pszString = tmpCst.GetBuffer();//debug 模式会存在问题 (LPSTR)(LPCTSTR)TMP_MbcsToUtf8((LPSTR)(LPCTSTR)m_strDatabaseFilePath);// = UnicodeToAscii(m_strDatabaseFilePath);
		if (NULL == pszString)
			throw - 3;
		//执行创建或打开SQLite3数据库指令
