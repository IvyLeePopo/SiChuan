...
public:
    ...
    QString	FormatSendData2Json( const CDVJsonDebitCheck* pDVJsonDebitCheck);
    ...
    QString FormatSendData2Json( const CDVJsonDebitSecond* pDVJsonDebitSecond);
...
private:
       //该函数不需要外部调用，屏蔽且并不实现
        QString	FormatSendData2Json(const CDVJsonHeart* pDVJsonHeart){return "";}
        //该函数不需要外部调用，屏蔽且并不实现
        ...
        CDVJsonDebitCheck* fnAnalysisStringForDebitCheck(const QString& strResponseResult);
