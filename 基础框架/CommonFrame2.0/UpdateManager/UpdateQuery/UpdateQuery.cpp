...
bool UpdateQuery::parseUpdateInfo(QString updateStr, QList<ST_UPDATE_INFO_FROM_HTTP> &result)
{
  ...
  for (int i = 0; i < infosVal.size(); ++i)
  {
    ...
    memset(&ui,0,sizeof(ST_UPDATE_INFO_FROM_HTTP));
    ui.type = 2;
    if (oneInfo["infoNo"].type() == Json::stringValue )
    {
         QString infoNo = oneInfo["infoNo"].asCString();
         memcpy(ui.infoNo,infoNo.toLocal8Bit().data(),infoNo.toLocal8Bit().length());
    }
    else
      return false;
    if (oneInfo["infoFileName"].type() == Json::stringValue )
    {
        QString infoFileName = oneInfo["infoFileName"].asCString();
        memcpy(ui.infoFileName,infoFileName.toLocal8Bit().data(),infoFileName.toLocal8Bit().length());
    }
    else
      return false;
    if (oneInfo["infoVersion"].type() == Json::stringValue )
    {
        QString infoVersion = oneInfo["infoVersion"].asCString();
        memcpy(ui.infoVersion,infoVersion.toLocal8Bit().data(),infoVersion.toLocal8Bit().length());
    }
    else
      return false;
    ...
    {
        QString infoFileMD5 = oneInfo["md5"].asCString();
        memcpy(ui.infoFileMD5,infoFileMD5.toLocal8Bit().data(),infoFileMD5.toLocal8Bit().length());
    }
    else
      return false;
    if(rootN["updateCode"].type()== Json::stringValue )
    {
        QString updateCode = rootN["updateCode"].asCString();
        memcpy(ui.updateCode,updateCode.toLocal8Bit().data(),updateCode.toLocal8Bit().length());
    }
    else
      return false;
...
QString UpdateQuery::generateQueryInfo()
{
  ...
  rootN["system"] = Json::Value(2);//linux
  ...
  for(it=dllNameList.begin(); it != dllNameList.end(); ++it)
  {
    ...
    QString dllVersion =myConfigFileOperate.parseVersionInfo(fileNameTemp);
