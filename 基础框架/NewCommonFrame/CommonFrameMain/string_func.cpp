...
std::list<std::string> Split_str (  std::string& src_str, char sep_ch )
{
	if ( src_str.empty() )
	{
		return std::list<std::string>();
	...
	while ( true )
	{
		size_t pos = src_str.find ( sep_ch );
		if ( pos == std::string::npos )
		{
			l_str.push_back ( src_str );
			break;
		}
		std::string cell = src_str.substr ( 0, pos );
		l_str.push_back ( cell );
		src_str.replace ( 0, pos + 1, "" );
	}
	return l_str;
}
